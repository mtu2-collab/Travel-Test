...
export function DestinationDetailPage() {
  ...
  const completeTrip = async () => {
    await markTripComplete(id, notes, dateVisited)
    await refreshUser()
    setOpen(false)
  }

  if (loading) return <Skeleton className="h-96 rounded-2xl" />
  if (error || !destination) return <div className="rounded-xl bg-red-50 p-3 text-sm">{error || 'Could not load'} <button className="underline" onClick={refetch}>Retry</button></div>
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex gap-2"><Button onClick={() => toggleWishlist(id)}>{saved ? 'Saved' : 'Save to Wishlist'}</Button><Button variant="ghost" onClick={() => setOpen(true)}>I&apos;ve Been Here</Button></div>
      ...
      <div className="mt-4 rounded-2xl bg-white p-3">
        ...
        <p className="mt-1 text-sm">{destination.teaserItinerary[0]}</p>
        <div className="mt-2 rounded-xl bg-navy/10 p-2 text-xs text-navy/50 blur-[1px]">{destination.teaserItinerary[1]} · {destination.teaserItinerary[2]}</div>
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <h3 className="font-semibold">Mark trip complete</h3>
        ...
        <Button className="mt-3 w-full" onClick={completeTrip}>Save</Button>
      </Modal>
    </div>
  )
}
